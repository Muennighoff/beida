...
def prepare_batch_data_hm(batch_records, num_choice, pad_id, task_index, task_num):
    """
    prepare batch data for HM finetuning task
    """
    batch_input_ids = []
    batch_input_pos = []
    batch_seg_ids = []
    ...
    # Binary Labels for CrossEntropy - Modified from prepare_batch_data_vcr
    binary_labels = np.array(batch_labels).astype("float32").reshape([-1, 1])
